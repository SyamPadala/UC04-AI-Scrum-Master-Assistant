...
| A14 | FR-02, FR-05, FR-09 | The day closes when the summary job runs. A member who messages after that is told the stand-up is closed and to speak to the Scrum Master; nothing is written to the tracker and they are counted as missed. The day closes whether or not the summary built successfully, because the day's record has been taken either way. Decided 22 Sep 2026. Only the **scheduled** summary closes the day: a manual `run` of any job is isolated from the scheduled cycle — it takes no run claim, so it neither closes the stand-up nor uses up the scheduled job, and a manual participation count is reported but not saved. Decided 23 Sep 2026 |
| A15 | FR-04 (Jira comment) | An update naming no work item has no ticket to comment on. With the Jira tracker it is recorded as a comment on one configured "Daily stand-up log" issue (`JIRA_STANDUP_ISSUE_KEY`, SCRUM-23 on the delivered site), kept out of sprints. Chosen over dropping it (loses what the member said) or refusing it (FR-02 forbids requiring a structured format). Decided 24 Sep 2026 |
